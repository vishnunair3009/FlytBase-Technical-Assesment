...
def generate_mission_file(filename="missions.txt", num_drones=10, # Default to 10 drones
    ...
    # Ensure number of converging drones doesn't exceed total drones minus the primary
    ...
    if num_drones > 0:
        ...
        if os.path.exists(filename):
            open(filename, 'w').close() # Clear existing file
    ...
    for i in range(num_converging_drones):
        ...
        # Ensure primary drone has at least one waypoint to converge towards
        if not primary_mission_details.get('waypoints') or not primary_mission_details['waypoints']:
             ...
             continue # Skip to the next converging drone
        ...
        # Calculate the approximate time the primary drone reaches this waypoint
        # This is a simplification; actual time depends on speed between waypoints
        ...
        # Let's simplify: start the converging drone's mission such that it reaches the target point roughly halfway through its own mission duration
        ...
        for k in range(conv_waypoints_count):
            if k == idx_for_target_in_conv_path:
                # Place the target primary waypoint directly in the converging drone's path
                conv_waypoints_list.append(target_primary_wp)
            else:
                # Generate random waypoints around the target point for other waypoints
        ...
        # Ensure there's at least one waypoint if conv_waypoints_count was 0
...
if __name__ == "__main__":
    # --- Option 1: Generate Random Missions with Converging Drones ---
    generate_mission_file(
        filename="random_collision_missions.txt",
        num_drones=10, # Total number of drones
        num_converging_drones=3, # Number of drones designed to converge with the primary
    ...
    # --- Option 2: Generate Guaranteed Collision Mission (for specific testing) ---
    # Uncomment the line below to generate a file with a guaranteed collision
    # generate_guaranteed_collision_mission(filename="guaranteed_collision_missions.txt",
    #                                       collision_point=(50, 50, 25), # The point where they will collide
    #                                       collision_time_offset_seconds=30, # Time into the mission when collision occurs
    #                                       mission_duration_seconds=60) # Total duration of the mission
